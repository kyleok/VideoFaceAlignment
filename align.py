...
def align_crop_image(image, landmarks, transform_size=256):
    ...
    if enable_padding and max(pad) > border - 4:
        ...
        mask = np.maximum(
            1.0
            - np.minimum(
                np.float32(x) / (pad[0] + 1e-12),
                np.float32(w - 1 - x) / (pad[2] + 1e-12),
            ),
            1.0
            - np.minimum(
                np.float32(y) / (pad[1] + 1e-12),
                np.float32(h - 1 - y) / (pad[3] + 1e-12),
            ),
        )
...
def main():
    ...
    parser.add_argument(
        "--input-dir", type=str, required=True, help="set input image directory"
    )
    ...
    if args.output_dir:
        ...
        output_dir = osp.join(
            osp.split(input_dir)[0], "{}_aligned".format(osp.split(input_dir)[1])
        )
    ...
    input_images = [
        osp.join(input_dir, dI)
        for dI in os.listdir(input_dir)
    ...
    for img_file in tqdm(
        input_images, desc="Preprocess {} images".format(len(input_images))
    ):
        # Open input image
        ...
        # Save output image
        cv2.imwrite(
            osp.join(output_dir, osp.split(img_file)[-1]),
